...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
		averageSpeed := (float64(action) / duration) * kmhInMsec
      return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(averageSpeed, 2) / height) * walkingSpeedHeightMultiplier) * duration * minInH)
}
